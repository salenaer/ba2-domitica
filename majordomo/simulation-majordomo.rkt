;-----------------------------------------------------------------------------------------------------------------------
;------------------------------------------------ simulation majordomo -------------------------------------------------
;-----------------------------------------------------------------------------------------------------------------------


;dit is de file die de simulatie opstart
#lang racket
(require "basic.rkt")
...
(define (make-majordomo simulation)  
  ...
  (let* (
         (stewards empty-list); in stewards zit een lijst van stewards-ids gekoppelt aan hun racket-object
         ...
         (raspberry-port-pair 0) ;een verbinding met de raspberry pi om de simulatie van 
         )
    
    (let-values (((in out) (tcp-accept listener)))
      (set! raspberry-port-pair (make-port-pair in out)))
    
    ;setters-------------------
    (define (add-steward steward-name)
      (my-write (args->string (list "add-steward" steward-name)) raspberry-port-pair)
      (let-values (((in out) (tcp-accept listener)))
        (set! stewards (conjoin (majordomo-node steward-name in out) stewards))))
    
    (define (erase-steward-majordomo steward)
      (my-write (args->string (list "remove-steward" steward)) raspberry-port-pair)
    ...
    (define (add-device steward-name device protocol product-id)
      (let ((steward (find-steward steward-name)))
        (if steward
            (begin (my-write (args->string (list "add-device" device (number->string protocol) product-id)) (get-port-pair steward))
                   (equal? (my-read (get-port-pair steward)) "device added"))
            (error "non-existing steward" steward-name))))
        ...
        (wait-for-free (lambda ()
                         (if on 
                             (begin (my-write (args->string (list "turn-on" device))(get-port-pair steward))
                                    (let ((result (my-read (get-port-pair steward))))
                                              (display result)
                                             (equal?  result "turned-on")))  
    ...
    (define (get-values stewards)
      (unless (none-left? stewards)
        (display "asking values of ")(display (get-name (first stewards)))
